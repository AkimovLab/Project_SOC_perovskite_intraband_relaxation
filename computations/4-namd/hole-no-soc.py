...
params["states"] = []
minband = 7; HOMO=36; LUMO=HOMO+1
params["active_space"] = range(minband,LUMO+1)
ap = list(reversed(range(minband,LUMO)))

cnt = 0
for j in ap:
    b = []
    for k in range(minband,LUMO):
       if k == j:
          b.append(k)
          b.append(-LUMO)
       else:
          b.append(k)
          b.append(-k)
    params["states"].append(["VBM-"+str(cnt),b, 0.00])
    cnt += 1
